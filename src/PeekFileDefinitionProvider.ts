...
export default class PeekFileDefinitionProvider implements vscode.DefinitionProvider {
  ...
  getComponentName(position: vscode.Position): String[] {
    ...
    this.targetFileExtensions.forEach(ext => {
      ...
      const selectedTextSplit = configParams.get('selectedTextSplitOnlyCapitalize') as Boolean
      if (!selectedTextSplit) {
        possibleFileNames.push(altName[0].toLowerCase() + altName.substring(1) + ext)
      }
      ...
      paths => {
        ...
        if (filePaths.length) {
          ...
          filePaths.forEach(filePath => {
            allPaths.push(
              new vscode.Location(vscode.Uri.file(`${filePath.path}`), new vscode.Position(0, 1))
            )
          })
          return allPaths
        } else {
